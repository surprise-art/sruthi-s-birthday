...
const HAPPY_BIRTHDAY = document.querySelector('.birthday-text__happy-birthday')
const NAME_TEXT = document.querySelector('.birthday-text__name')
...
// Typing animation for birthday text - only when candles light up
const TYPING_TL = () => {
  const happyText = "Happy Birthday"
  const nameText = "Sruthi Prasanna"
  
  // Clear text content initially
  HAPPY_BIRTHDAY.textContent = ""
  NAME_TEXT.textContent = ""
  
  const tl = timeline()
  
  // Show birthday text container
  tl.to(BIRTHDAY_TEXT, { opacity: 1, duration: 0.5 })
  
  // Type "Happy Birthday"
  for (let i = 0; i <= happyText.length; i++) {
    tl.to(HAPPY_BIRTHDAY, {
      duration: 0.1,
      onComplete: () => {
        HAPPY_BIRTHDAY.textContent = happyText.substring(0, i)
      }
    })
  }
  ...
  // Type "Sruthi Prasanna"
  for (let i = 0; i <= nameText.length; i++) {
    tl.to(NAME_TEXT, {
      duration: 0.1,
      onComplete: () => {
        NAME_TEXT.textContent = nameText.substring(0, i)
      }
    })
  }
...
const MASTER_TL = timeline({
  ...
  .add(TYPING_TL(), 'FLAME_ON') // Start typing when candles light up
  .add(LIGHTS_OUT(), 'LIGHTS_OUT')
  .add(FADE_TEXT(), 'LIGHTS_OUT') // Fade text when lights go out
